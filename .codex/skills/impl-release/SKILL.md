...
3. 作業開始前に `make release-target-check VERSION=vX.Y.Z` を実行し、公開済み release line から見て自然な次版であることを確認する。
4. `v0.17.6` の次に `v0.18.7` を指定するような不自然な飛び番は停止し、`v0.17.7` か `v0.18.0` かをユーザーに確認する。
5. `KML_RELEASE_ALLOW_VERSION_LINE_OVERRIDE=1` は、修正リリースなどの理由をユーザーが明示承認した場合だけ使う。
...
9. `make release` が失敗した場合、代替の tag / publish コマンドで迂回しない。状態を調査し、修正してから同じフローへ戻る。
