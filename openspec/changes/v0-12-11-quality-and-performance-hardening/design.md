...
`v0.12.11`は「次の配布準備」ではなく、`v0.12.9`後の品質定着 patch として扱う。
...
`v0.12.11` は「精度優先」を最上位原則に置く。

## Goals / Non-Goals

**Goals:**

- `v0.12.11` の最重要目標を、外部品質と内部保守性の両輪に戻す
- 速度管理は、精度維持の前提で行い、回帰判断を規約化する
- `v0.12.11` 後に次版で着手すべき改善を明確化する
...
- `v0.12.9` の quality score 運用を踏襲し、`v0.12.11` でも同一の採点フレーム（100点満点、threshold 90、`technical_hard_blockers` 判定）で release 判断材料を残す
- `v0.12.11` の成果物は tasks と evidence を残し、次版へ流す follow-up と分離する

## Quality Scoring

- `v0.12.11` では、`v0.12.9 public-confidence score` のカテゴリを継続採用する
