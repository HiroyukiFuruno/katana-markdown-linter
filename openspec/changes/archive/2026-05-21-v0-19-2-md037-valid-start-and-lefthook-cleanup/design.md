...
### Decision 1: `valid_start` を「句読点直後の開始」と「通常強調の閉じ」を分離して判定する

PR #98 の連結バグの真因は、通常強調を閉じている `**` を、後続の別強調範囲の開始候補として再利用していた点にある。一方で、`Hello.* spaced *.` のような句読点直後のスペース付き強調は MD037 の対象に戻す必要がある。`valid_start` を以下の組み合わせで判定する:

1. 直前が行頭 OR 空白 OR `([{"'` → opener 候補（従来通り）
2. 直前が `.!?,;:` 等の句読点 → MD037 のスペース付き強調開始として opener 候補
3. 同種・同長の直前 marker が通常強調の開始として成立しており、現在 marker がその閉じ記号に見える場合 → opener 候補から除外

これにより `Hello.* spaced *` の `*` は (2) で opener 候補になり MD037 が検出する。`**Note:**` の閉じ `**` は (3) で除外され、後続独立強調との誤マッチを防ぐ。
...
- `src/**` 配下の Rust 変更も拾えるように、root 用 `*.{rs,toml,lock}` と配下用 `**/*.{rs,toml,lock}` の両方を指定する。
...
- **Risk:** lefthook 構成変更によりローカル pre-push が誤動作 → Mitigation: 変更後に `lefthook run pre-push --file <path>` をローカルで実行し、Markdown のみ変更時に skip、Rust/TOML/lock 変更時に check が走ることを確認する。
