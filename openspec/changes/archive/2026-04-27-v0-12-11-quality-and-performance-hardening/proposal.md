...
- `make perf-check` は速度退行を報告するが、今回の `v0.12.11` 方針では「精度優先」を崩さない形で回帰管理を強化したい
...
- `v0.12.9` と同様に、`v0.12.11` でも `public confidence score`（100点満点）を算出する。カテゴリ・閾値・`hard blocker` を明示し、公開用の品質判断材料として残す
- 3本柱を分離した tasks と DoD を追加し、`v0.12.11` で完了条件を明示する
