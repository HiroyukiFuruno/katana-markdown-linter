...
- `v0.12.3`: patch release として lint 精度、速度（performance）、単体テスト（UT）/結合テスト（IT）拡充を優先する。
...
| P0 | Precision, performance, and test hardening for `v0.12.3` | `v0-12-3-precision-performance-test-hardening` | Improves lint precision, measured performance, and single-rule / CLI-level test coverage before MCP Registry work resumes. |
...
1. Complete `v0-12-3-precision-performance-test-hardening` first; it improves lint precision, measured speed, and test depth before public MCP distribution work.
2. Apply `mcp-registry-and-distribution-planning` after `v0.12.3`; it decides whether MCPB, OCI, or another package type is the primary path.
...
- `design-debt`: `MD034` と `MD059` の inline code / HTML 周辺は現時点の回帰 fixture で固定する。より深い token parser 化は `v0.12.3` の品質改善枠で扱う。
