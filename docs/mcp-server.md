...
It exists for agents and editors that want structured diagnostics and rule metadata without using the CLI as a subprocess.
...
| --- | --- | --- |
| `check_text` | none | Lints supplied Markdown content and returns structured diagnostics. |
| `fix_text` | none | Applies safe fixes and returns fixed content without writing files. |
| `config_validate` | none | Validates a markdownlint-compatible JSON config object. |
...
The core library remains MCP-free. `kml-mcp` is an adapter that depends on the public Rust API.
