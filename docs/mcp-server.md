...
Add the server to `~/.codex/config.toml`:
    ...
    [mcp_servers.kml]
    command = "npx"
    args = ["--yes", "katana-markdown-linter@0.19.5", "kml-mcp", "--workspace-root", "/absolute/path/to/workspace"]
...
`uvx` is also valid:
    ...
    [mcp_servers.kml]
    command = "uvx"
    args = ["--from", "katana-markdown-linter==0.19.5", "kml-mcp", "--workspace-root", "/absolute/path/to/workspace"]
    default_tools_approval_mode = "prompt"
...
Equivalent JSON form:
    ...
    claude mcp add-json kml \
      '{"type":"stdio","command":"npx","args":["--yes","katana-markdown-linter@0.19.5","kml-mcp","--workspace-root","/absolute/path/to/workspace"]}'
...
to each GitHub Release:
    ...
    katana-markdown-linter-0.19.5.mcpb
    katana-markdown-linter-0.19.5.mcpb.sha256

The committed `server.json` is the source metadata. During release,
`just VERSION=v0.19.5 mcp-server-json` renders `target/mcpb/server.json` with
...
Validate the rendered metadata:
    ...
    just VERSION=v0.19.5 server-json-validate
