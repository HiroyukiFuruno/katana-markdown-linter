...
to each GitHub Release:
    ...
    katana-markdown-linter-0.16.2.mcpb
    katana-markdown-linter-0.16.2.mcpb.sha256

The committed `server.json` is the source metadata. During release,
`make mcp-server-json VERSION=v0.16.2` renders `target/mcpb/server.json` with
...
Validate the rendered metadata:
    ...
    make server-json-validate VERSION=v0.16.2
